(function() {
    ...
    angular.module("firebase.starter", ["firebase"])
        .provider("fireStarter", FireStarter);
    ...
    function FireStarter() {
        ...
        self.$get = ["$timeout", "$window", "$firebaseAuth", "$firebaseObject",
            "$firebaseArray", "$q", "$log",
            function($timeout, $window, $firebaseAuth, $firebaseObject, $firebaseArray, $q, $log) {
                ...
                function rootRef() {
                    return new self._window.Firebase(self.getRoot());
                }

                function checkParams() {
                    var typeOptions = ["auth", "array", "object", "geo", "ref"];
                    if (!angular.isString(self.getRoot())) {
