(function() {
    ...
    angular.module("firebase.starter", ["firebase"])
        .provider("fireStarter", FireStarterService);


    function FireStarterService() {
    ...
    FireStarter.prototype = {
        constructRoot: function(path) {
            var root, self = this;
            if (path) {
                self._timeout(function() {
                    return new self._window.Firebase(path);
                }).then(function(res) {
                    root = res;
										return root;
                }).catch(function(err) {
