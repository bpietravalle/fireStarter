...
describe('$firebaseArray', function() {
    ...
    var arr, $firebaseArray, $utils, $timeout, testutils, fireStarter;
    beforeEach(function() {
        module('fireStarter.services');
        module('firebase');
        module('testutils');
        inject(function(_$firebaseArray_, $firebaseUtils, _$timeout_, _testutils_, _fireStarter_) {
    ...
    //         expect(typeof arr.base()[k]).toBe('function');
    //         i++;
    //       });
    ...
    describe('add', function() {
        ...
        //     var factory = $firebaseArray.$extend({
        //         added: addPromise
        //     });
						// var ref = stubRef();
        //     arr = extendArray(undefined, factory, ref); 
        ...
        //     expect(arr.base()[0].name).toBe('James');
        // });
        ...
        //     });
            ...
            it('should store priorities', function() {
              ...
              expect(arr.base().length).toBe(5);
              for(var i=1; i <= 5; i++) {
                expect(arr.base()[i-1].$priority).toBe(i);
              }
            });
      ...
      describe('remove', function() {
        ...
        it('should work on a query', function() {
          ...
          var arr = stubArray(undefined, query, true);
          flushAll(arr.ref());
          var key = arr.keyAt(1);
          arr.remove(1).then(whiteSpy, blackSpy);
          flushAll(arr.ref());
      ...
      describe('indexFor', function() {
        ...
        it('should not show up after removing the item', function() {
          ...
          arr.base().$$process('child_removed', rec);
      ...
      describe('loaded', function() {
        ...
        it('should resolve to the array', function() {
          ...
          arr.loaded().then(spy);
          flushAll();
          expect(spy).toHaveBeenCalledWith(arr.base());
        });
        ...
        it('should reject when error fetching records', function() {
          ...
          var ref = stubRef();
          ref.failNext('on', err);
          var arr = stubArray(null, ref, true);
          arr.loaded().then(whiteSpy, blackSpy);
          flushAll(ref);
          expect(whiteSpy).not.toHaveBeenCalled();
          expect(blackSpy).toHaveBeenCalledWith(err);
      ...
      describe('watch', function() {
        it('should get notified when $$notify is called', function() {
          var spy = jasmine.createSpy('watch');
          arr.watch(spy);
          arr.base().$$notify('child_removed', 'removedkey123', 'prevkey456');
      ...
      describe('$$added', function() {
        ...
        it('should accept a primitive', function() {
          var res = arr.base().$$added(testutils.snap(true, 'newPrimitive'), null);
        ...
        //   }));
        //   var res = arr.base().$$added(testutils.snap(STUB_DATA.a));
      ...
      describe('$$updated', function() {
        it('should return true if data changes', function() {
          var res = arr.base().$$updated(testutils.snap('foo', 'b'));
          expect(res).toBe(true);
        });

        it('should return false if data does not change', function() {
          var i = arr.indexFor('b');
          var res = arr.base().$$updated(testutils.snap(arr.base()[i], 'b'));
          expect(res).toBe(false);
        });
        ...
        it('should update local data', function() {
          ...
          expect(arr.base()[i]).toEqual(jasmine.objectContaining({'$value': 'foo'}));
        });
        ...
        it('should preserve ids', function() {
          ...
          arr.base().$$updated(testutils.snap({foo: 'bar'}, 'b'));
        ...
        it('should set priorities', function() {
          var pos = arr.indexFor('b');
          expect(pos).toBeGreaterThan(-1);
          arr.base().$$updated(testutils.snap({foo: 'bar'}, 'b', 250));
    ...
    //       delete rec.foo;
    //       arr.base().$$updated(testutils.snap($utils.toJSON(rec), 'a'));
    //       expect(rec.aString).toBe(STUB_DATA.a.aString);
    //       expect(rec.foo).toBe('foo');
    //     });
      });
      ...
      describe('$$moved', function() {
        it('should set $priority', function() {
          ...
          arr.base().$$moved(testutils.snap($utils.toJSON(rec), 'c', 999), 'd');
          expect(rec.$priority).toBe(999);
        });

        it('should return true if record exists', function() {
          var rec = arr.getRecord('a');
          var res = arr.base().$$moved(testutils.snap($utils.toJSON(rec), 'a'), 'c');
          expect(res).toBe(true);
        });

        it('should return false record not found', function() {
          var res = arr.base().$$moved(testutils.snap(true, 'notarecord'), 'c');
          expect(res).toBe(false);
        });
      });
      ...
      describe('$$removed', function() {
        it('should return true if exists in data', function() {
          var res = arr.base().$$removed(testutils.snap(null, 'e'));
          expect(res).toBe(true);
        });

        it('should return false if does not exist in data', function() {
          var res = arr.base().$$removed(testutils.snap(null, 'notarecord'));
          expect(res).toBe(false);
        });
      });
      ...
      describe('$$notify', function() {
        ...
        it('should pass an object containing key, event, and prevChild if present', function() {
          ...
          arr.watch(spy);
          arr.base().$$notify('child_added', 'e', 'd');
    ...
    //     /////////////// ADD
        it('should add to local array', function() {
          var len = arr.base().length;
          var rec = arr.base().$$added(testutils.snap({hello: 'world'}, 'addz'), 'b');
          arr.base().$$process('child_added', rec, 'b');
        ...
        it('should position after prev child', function() {
          var pos = arr.indexFor('b');
          expect(pos).toBeGreaterThan(-1);
          var rec = arr.base().$$added(testutils.snap({hello: 'world'}, 'addAfterB'), 'b');
          arr.base().$$process('child_added', rec, 'b');
          expect(arr.keyAt(pos+1)).toBe('addAfterB');
        });

        it('should position first if prevChild is null', function() {
          var rec = arr.base().$$added(testutils.snap({hello: 'world'}, 'addFirst'), null);
          arr.base().$$process('child_added', rec, null);
          expect(arr.keyAt(0)).toBe('addFirst');
        });

        it('should position last if prevChild not found', function() {
          var len = arr.base().length;
          var rec = arr.base().$$added(testutils.snap({hello: 'world'}, 'addLast'), 'notarealkeyinarray');
          arr.base().$$process('child_added', rec, 'notrealkeyinarray');
    ...
    //       var prevChild = arr.base().$$getKey(arr.base()[index -1]);
    //       spy.calls.reset();
    //       arr.base().$$process('child_added', arr.getRecord('e'), prevChild);
    //       expect(spy).not.toHaveBeenCalled();
    //     });
    ...
    //       var arr = stubArray(STUB_DATA, $firebaseArray.$extend({ $$notify: spy }));
    //       spy.calls.reset();
    //       arr.base().$$updated(testutils.snap({hello: 'world'}, 'a'));
    //       arr.base().$$process('child_changed', arr.getRecord('a'));
    //       expect(spy).toHaveBeenCalled();
    //     });
    ...
    //     ///////////////// MOVE
        it('should move local record', function() {
          ...
          arr.base().$$moved(testutils.refSnap(testutils.ref('b')), 'c');
          arr.base().$$process('child_moved', arr.getRecord('b'), 'c');
        ...
        it('should position at 0 if prevChild is null', function() {
          ...
          arr.base().$$moved(testutils.snap(null, 'b'), null);
          arr.base().$$process('child_moved', arr.getRecord('b'), null);
    ...
    //       var prevChild = arr.base().$$getKey(arr.base()[index - 1]);
    //       spy.calls.reset();
    //       arr.base().$$process('child_moved', arr.getRecord('e'), prevChild);
    //       expect(spy).not.toHaveBeenCalled();
    //     });
    ...
    //       arr.base().$$process('child_removed', arr.getRecord('e'));
    //       expect(spy).toHaveBeenCalled();
    //     });
    ...
    //       var arr = stubArray(STUB_DATA, $firebaseArray.$extend({ $$notify: spy }));
    //       spy.calls.reset();
    //       arr.base().$$process('child_removed', {$id:'f'});
    //       expect(spy).not.toHaveBeenCalled();
    //     });
    ...
    //     //////////////// OTHER
        it('should throw an error for an unknown event type',function(){
          ...
          expect(function(){
            arr.base().$$process('unknown_event', arr.getRecord('e'));
          }).toThrow();
        });
    ...
    function addAndProcess(arr, snap, prevChild) {
			//added $
        arr.base().$$process('child_added', arr.base().$$added(snap, prevChild), prevChild);
    }
